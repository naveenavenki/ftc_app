...
public class BoKAutoCommon implements BoKAuto {
    ...
    public void initSoftware(LinearOpMode opMode, BoKHardwareBot robot, BoKAlliance redOrBlue) {
        ...
        robot.partLiftGateServo.setPosition(robot.INITIAL_SERVO_POS_PART_GATE);
    ...
    protected void shootBall(LinearOpMode opMode, BoKHardwareBot robot, double shooterMotorsPower, double waitForSec) throws InterruptedException {
        ...
        if (opMode.opModeIsActive()) {
            ...
            robot.setPowerToShooterMotors(shooterMotorsPower);   // start the ball shooter
    ...
    protected void moveForward(LinearOpMode opMode, BoKHardwareBot robot, double leftPower, double rightPower, double inchesForward, double waitForSec) throws InterruptedException {
        ...
        if (opMode.opModeIsActive()) {
            ...
            runTime.reset();
            while (opMode.opModeIsActive() && (robot.getDTCurrentPosition() == false) && (runTime.seconds() < waitForSec)) {
                //opMode.telemetry.update();
    ...
    protected void proportionalLineFollower(LinearOpMode opMode, BoKHardwareBot robot, boolean left, double distanceToWall) throws InterruptedException
    {
        ...
        if (opMode.opModeIsActive()) {
            ...
            //opMode.telemetry.update();
            Log.v("BOK", "Line Final: " + String.format("%.2f", robot.odsSensor.getLightDetected()) + " gyro: " + robot.gyroSensor.getIntegratedZValue() + " d: " + robot.rangeSensor.cmUltrasonic());
        } // if (opModeIsActive())
    }
    ...
    protected void goForwardToWall(LinearOpMode opMode, BoKHardwareBot robot, double targetDistance, double waitForSec) throws InterruptedException
    {
        ...
        if (opMode.opModeIsActive()) {
            ...
            Log.v("BOK", "goForward: " + distance + ", a: " + String.format(".2f", current_alpha) + ", ang: " + robot.gyroSensor.getIntegratedZValue());
    ...
    protected void goForwardTillBeacon(LinearOpMode opMode, BoKHardwareBot robot, double targetDistance, double waitForSec) throws InterruptedException
    {
        double distance;
        // Ensure that the opmode is still active
        if (opMode.opModeIsActive()) {
            ...
            Log.v("BOK", "goForward: " + distance + ", ang: " + robot.gyroSensor.getIntegratedZValue());
            // stop when near the white line and the distance < target distance
            while (opMode.opModeIsActive() && (distance > targetDistance) && (runTime.seconds() < waitForSec)) {
                distance = robot.rangeSensor.cmUltrasonic();
     ...
     * @return
     */
    private boolean onHeading(LinearOpMode opMode, BoKHardwareBot robot, double speed, double angle, double PCoeff) {
        double   error ;
        double   steer ;
